...
describe('AI Service', () => {
  describe('detectCrisis', () => {
    it('should detect crisis keywords', () => {
      const review = 'This is terrible service and I am considering legal action'
  ...
  describe('filterPolicy', () => {
    it('should detect PII in review', () => {
      ...
      expect(result.violations).toContain('Contains PII (email, phone, SSN, or credit card)')
    })
    ...
    it('should detect length violations', () => {
      ...
      expect(result.passed).toBe(false)
      expect(result.violations).toContain('Exceeds maximum length of 500 characters')
    })
