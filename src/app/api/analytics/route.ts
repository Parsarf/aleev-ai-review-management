...
export async function GET(request: NextRequest) {
  try {
    ...
    const targetBusinessIds = query.businessId ? [query.businessId] : businessIds
    ...
    const reviewsWithReplies = reviews.filter(r => r.reply?.status === 'SENT')
    ...
    // Rating trends (daily)
    const ratingTrends = await getRatingTrends(targetLocationIds, startDate, query.days)

    // Sentiment trends (daily)
    const sentimentTrends = await getSentimentTrends(targetLocationIds, startDate, query.days)
...
async function getRatingTrends(locationIds: string[], startDate: Date, days: number) {
  const trends = []
...
async function getSentimentTrends(locationIds: string[], startDate: Date, days: number) {
  const trends = []
...
function getCommonIssues(reviews: any[]) {
  ...
  const issueKeywords = [
    'service', 'food', 'wait', 'time', 'staff', 'clean', 'dirty', 'slow',
    'rude', 'cold', 'hot', 'price', 'expensive', 'cheap', 'quality',
    'atmosphere', 'noise', 'crowded', 'parking', 'location'
  ]
