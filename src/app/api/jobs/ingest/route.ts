...
export async function POST(request: NextRequest) {
  try {
        ...
        for (const [platform, config] of Object.entries(platformAccounts)) {
          ...
          try {
            ...
            if (!adapter.isConnected(config)) {
              console.log(`Platform ${platform} not connected for location ${location.id}`)
              continue
            }
            ...
            for (const reviewData of reviews) {
              try {
                ...
                console.error(`Error processing review ${reviewData.platformId}:`, reviewError)
                totalErrors++
              }
            }
          } catch (platformError) {
            console.error(`Error processing platform ${platform} for location ${location.id}:`, platformError)
            totalErrors++
          }
        }
      } catch (locationError) {
        console.error(`Error processing location ${location.id}:`, locationError)
        totalErrors++
      }
    }
    ...
    console.log(`Ingestion job completed. Ingested: ${totalIngested}, Errors: ${totalErrors}`)
