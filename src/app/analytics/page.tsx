  ...
  if (!data) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-600">Analytics data will appear here once you have reviews.</p>
      </div>
    )
  }

  const platformData = Object.entries(data.distribution.platforms).map(([name, value]) => ({
    name,
    value
  }))

  const statusData = Object.entries(data.distribution.statuses).map(([name, value]) => ({
    name: name.replace('_', ' '),
    value
  }))
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.kpis.avgRating.toFixed(1)}</div>
            <p className="text-xs text-muted-foreground">
              Out of 5 stars
            </p>
          </CardContent>
        ...
        <CardContent>
          <div className="space-y-2">
            {data.commonIssues.map((issue, index) => (
              <div key={issue.keyword} className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-gray-600 w-8">{issue.count}</span>
                </div>
              </div>
            ))}
          </div>
