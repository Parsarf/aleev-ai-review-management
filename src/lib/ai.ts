...
const CRISIS_KEYWORDS = process.env.CRISIS_KEYWORDS?.split(',') || [
  'lawsuit', 'legal action', 'health department', 'unsafe', 'contaminated',
  'poisoned', 'terrible service', 'worst experience', 'never again',
  'horrible', 'disgusting'
]

// Banned phrases for policy filtering
const BANNED_PHRASES = process.env.BANNED_PHRASES?.split(',') || [
  'refund guaranteed', 'always', 'never', 'free meal', '100% satisfaction',
  'perfect', 'flawless'
]
